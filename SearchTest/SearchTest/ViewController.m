...
- (NSArray *)generateSectionsFromArray:(NSArray *)array withFilter:(NSString *)filterString
{
	...
	for (NSString *string in array) {
		...
		ALSection *section = nil;
		
		NSString *firstLetter = [string substringToIndex:1];
		
		if (![currentLetter isEqualToString:firstLetter]) {
			section = [[ALSection alloc] init];
...
-(NSArray<NSString *> *)sectionIndexTitlesForTableView:(UITableView *)tableView
{
	...
	for (ALSection *section in self.sectionsArray) {
...
{
    ALSection *sec = [self.sectionsArray objectAtIndex:section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ALSection *section = [self.sectionsArray objectAtIndex:indexPath.section];
